...
class _FavoritesPageState extends State<FavoritesPage>
  ...
  Future<void> _fetchAnimeDetails(List<String> ids) async {
    ...
    for (final id in ids) {
      ...
      try {
        ...
        if (media != null) {
          final anime = Anime.fromGraphQL(media);
          _animeDetails[id] = anime;
        }
      } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    // Filtered anime favorites
    final animeIds = favModel.animeFavoritesList.where((id) {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed:
            (isSongsTab ? songFaves.isNotEmpty : animeIds.isNotEmpty)
                ? _onCreatePlaylist
                : null,
          ),
          IconButton(
            ...
            onPressed: () {
              ...
              if (isSongsTab) {
                for (var m in filteredSongs) {
                  ...
                  buffer.writeln('• Anime: ${anime['title'] ?? ''}'
              ...
              children: [
                ...
                filteredSongs.isEmpty
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final key = ValueKey(m['song_id'] ?? i);
                    return Dismissible(
                      key: key,
                      ...
                      onDismissed: (_) {
                        context.read<FavoritesModel>().toggleFavorite(m);
                        if (mounted) setState(() {});
                      ...
                      child: songTile(filteredSongs[i]),
                    );
                  },
                ),
  ...
  Future<void> _showAddDialog(List<Map<String, dynamic>> items) async {
    ...
    final result = await showDialog<String>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          ElevatedButton(onPressed: () => Navigator.pop(context, pick), child: const Text('Add')),
        ],
      ),
    );
    if (result != null) {
  ...
  Widget songTile(Map<String, dynamic> m) {
    ...
    return ListTile(
      ...
      trailing: _selectedPlaylist != null
        ...
        onPressed: () {
          context.read<PlaylistsModel>().removeSongFromPlaylist(_selectedPlaylist!, m);
          if (mounted) setState(() {});
        },
      )
          : null,
  ...
  Widget animeTile(String id) {
    ...
    return ListTile(
      ...
      trailing: _selectedPlaylist != null
        ...
        icon: const Icon(Icons.delete),
        onPressed: () {
          context.read<PlaylistsModel>().removeAnimeFromPlaylist(_selectedPlaylist!, id);
          if (mounted) setState(() {});
        },
      )
          : null,
